...
class LegymPost:
    def __init__(self) -> None:
        ...
        self.headers.update(
            {"authorization": "Bearer " + self.token, "Organization": self.schoolId}
        )
    ...
    def limitation(self):
        ...
        limit_response = requests.post(
            url, headers=self.headers, data=json.dumps(self.limit_data), verify=False
        )
    ...
    def filtrateTargetActivity(self, activityList: list) -> list:
        ...
        for item in activityList:
            if (
                ...
                and item["name"].find(TargetActivities) != -1
            ):
                lst1.append(item["id"])
        return lst1
    ...
    def run_route(self) -> None:
        ...
        starttime = (
            datetime.datetime.now() - datetime.timedelta(minutes=random_time)
        ...
        response = requests.post(
            url, headers=self.headers, data=json.dumps(self.run_data), verify=False
        )  # 开始跑步发包
        print(response.text)
        return 0
